...
Один із тестів сторожує не код, а документацію. `tests/unit/test_schema.py` тримає [`checklist-v1.schema.json`](checklist-v1.schema.json) при §2 спеки: валідує схемою кожен `json`-блок цілого файла зі спеки та з [`checklist-format.md`](checklist-format.md), ганяє фікстури `tests/fixtures/` (валідні мусять пройти, невалідні — впасти) і окремо перевіряє унікальність `id`, якої JSON Schema виразити не може. Схема є похідним файлом, і без цього тесту її узгодженість зі спекою трималася б на памʼяті. Звідси два обмеження на документацію: контрприклади в спеці й авторському документі подаються неповними прикладами або прозою, а не цілими файлами, а невалідні фікстури ламають рівно одну клаузу контракту валідації кожна — інакше зникнення клаузи зі спеки нікого не потривожить.
